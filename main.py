# -*- coding: utf-8 -*-
"""
Created on Sun Nov  3 20:51:24 2024
...
if __name__ == '__main__':
    ...
    if way == '1':
        ...
        for byte in data:
            temp.append(byte)
            if len(temp) == 16:
                # change aes2 to aes128
                crypted_part = aes2.encrypt(temp, key)
        ...
        for byte in data:
            temp.append(byte)
            if len(temp) == 16:
                decrypted_part = aes2.decrypt(temp, key)
            ...
            if 0 < len(temp) < 16:
                ...
                for i in range(empty_spaces - 1):
                    temp.append(0)
                temp.append(1)
                decrypted_part = aes2.encrypt(temp, key)
